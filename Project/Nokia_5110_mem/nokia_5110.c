...
void LCD_GPIO_Init(void)
{
   ...
   GPIO_Init(LCD_PORT_CLK, &GPIO_InitStructure);
	 ...
	 GPIO_InitStructure.GPIO_Pin = LCD_PIN_VCC;
   GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
   GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
   GPIO_Init(LCD_PORT_VCC, &GPIO_InitStructure);
	 
	 GPIO_InitStructure.GPIO_Pin = LCD_PIN_LIG;
   GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
   GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
   GPIO_Init(LCD_PORT_LIG, &GPIO_InitStructure);
	 
	 GPIO_InitStructure.GPIO_Pin = LCD_PIN_GND;
   GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
   GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
   GPIO_Init(LCD_PORT_GND, &GPIO_InitStructure);
	 
	 GPIO_SetBits(LCD_PORT_VCC, LCD_PIN_VCC);               // set LCD_PIN_VCC; => VCC = 3.3v
   GPIO_ResetBits(LCD_PORT_GND, LCD_PIN_GND);             // clr LCD_PIN_GND; => GND = 0v
	 //GPIO_ResetBits(LCD_PORT_LIG, LCD_PIN_LIG);             // bat LED sang len
	 
	 GPIO_SetBits(LCD_PORT_LIG, LCD_PIN_LIG);               // tat LED chieu sang
